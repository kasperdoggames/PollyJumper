...
contract GameNFTToken is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Polly", "POLLY") {
        admin = msg.sender;
        setBaseURI("ipfs://");
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == admin,
            "GameNFTToken: Admin can perform this operation."
        );
        _;
    }
    ...
    modifier onlyAdminOrP2EGame() {
        require(
            msg.sender == p2eGame || msg.sender == admin,
            "GameNFTToken: Admin or P2EGame can perform this operation."
        );
        _;
    }
    ...
    function mint(uint256 _mintAmount) public onlyAdmin {
        ...
        require(
            ...
            "GameNFTToken: You can't mint that many in one go"
        );
        require(
            supply + _mintAmount <= maxSupply,
            "GameNFTToken: Not enough left for you to mint that many"
        );
    ...
    function mintOne(address playerAddress) public onlyAdminOrP2EGame {
        ...
        require(
            walletOfOwner(playerAddress).length == 0,
            "GameNFTToken: Don't be greedy, one NFT per person"
        );
